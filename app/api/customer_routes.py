...
async def register_customer(user_in: UserCreate, customer_profile_in: CustomerProfileCreate, db: Session = Depends(get_db)):
...
async def update_customer_profile(user_update: UserUpdate, customer_profile_update: CustomerProfileUpdate, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    ...
    try:
        user = await update_customer(db, current_user.id, user_update, customer_profile_update)
        customer_profile = db.query(CustomerProfile).filter(CustomerProfile.user_id == user.id).first()
        profile_response = customer_profile.default_address
        user_response = get_user_response(user)
